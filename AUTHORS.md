# Invoice³ - Authors

## Creator & Lead Developer

**Subash Natarajan**

- 💼 **LinkedIn**: [linkedin.com/in/subashn](https://www.linkedin.com/in/subashn/)
- 📧 **Email**: suboss87@gmail.com
- 🏆 **Role**: Full Stack Developer, AI/ML Engineer
- 🎯 **Project**: Built for LandingAI Financial Hackathon 2024

### Contributions
- 🏗️ System architecture and design
- 💻 Full-stack development (Backend + Frontend)
- 🤖 AI agent implementation (LangGraph + Gemini)
- 📊 Dashboard and UI/UX design
- 📝 Complete documentation
- 🚀 Production deployment setup

---

## Tech Stack Expertise

### Backend
- FastAPI - RESTful API development
- LangGraph - Multi-agent orchestration
- Google Gemini 2.0 Flash - LLM integration
- LandingAI ADE - Document extraction
- Python - Core development

### Frontend
- React 18 - Modern UI framework
- TypeScript - Type-safe development
- Tailwind CSS - Responsive design
- Shadcn/UI - Component library
- Vite - Fast build tooling

### AI/ML
- Agentic workflows with LangGraph
- LLM prompt engineering
- Document AI (OCR/extraction)
- Fraud detection algorithms
- RAG (Retrieval-Augmented Generation)

---

## Project Timeline

- **Development Period**: October - November 2024
- **Hackathon**: LandingAI Financial Hackathon 2024
- **Lines of Code**: 10,000+ (Backend + Frontend)
- **Documentation**: 5 comprehensive guides
- **Status**: Production-ready, open source

---

## Connect

Want to collaborate or learn more about this project?

- 📬 **Email**: suboss87@gmail.com
- 💼 **LinkedIn**: https://www.linkedin.com/in/subashn/
- 💻 **GitHub**: Check out other projects!

---

## Acknowledgments

Special thanks to:
- **LandingAI** - For the amazing ADE API and hackathon opportunity
- **Google** - For Gemini API access
- **LangChain Team** - For LangGraph framework
- **Open Source Community** - For the amazing tools and libraries

---

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

Copyright (c) 2024 Subash Natarajan

---

*"Building intelligent systems that make a difference."* - Subash Natarajan
